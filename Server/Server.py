...
class Server():
    ...
    def send_msg_to_room(self,conn,msg,client_name,client_joined_room):
        if self.see_msgs_room == client_joined_room:
    ...
    def admin_page(self):
        while True:
            ...
            if main_menu == "1": 
                ...
                os.system("cls")
                room_key = 0
                room_keys = list(self.rooms.keys())
                for room_num in room_keys:
                    print(f"{room_key}. {room_num}")
                    room_key += 1
                room_to_see = input("Which room to you want to see: ")
                try:
                    self.see_msgs_room = room_keys[int(room_to_see)]
                except:
                    print("!!Invalid Value!!")
                input("")
                self.see_msgs_room = None
